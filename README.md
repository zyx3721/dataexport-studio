...
- 可按所选字段升序或降序导出。
- 支持关系型数据库的自定义 SQL 导出，可执行单条只读 `SELECT` 或 `WITH ... SELECT` 查询，适用于多表连接、CTE 与聚合；MongoDB 支持图形化配置和自定义只读 JSON 聚合管道，可使用 `$lookup` 关联多个 Collection。
- 使用流式读取和 `openpyxl` 写入，默认最多导出 1,000,000 行；超过单工作表限制时自动新建工作表。
- Excel 对齐 CertFlow 审计日志格式：宋体、白底、仅表头加粗、轻量细网格和居中显示；列宽按前 200 行数据样本计算。
- 数据库原生的整数与小数会以 Excel 数字单元格导出；文本型编号仍保持文本，避免丢失前导零。
- 导出进度默认显示 `0%`；导出期间平滑更新，完成时显示 `100%`。

Oracle、写操作、管理 SQL 与凭据持久化不属于当前范围。
...
3. 在“查询方式”中选择“图形化配置”时，可填写一个或多个筛选条件，选择 `AND` 或 `OR` 连接关系；输入 `NULL` 可匹配空值。勾选“按字段排序”后可选择排序字段与方向。选择“自定义 SQL”时，先在右侧“目标数据库”选择实际执行查询的数据库，再填写单条只读 `SELECT` 或 `WITH ... SELECT` 查询；应用会重新连接到该数据库，图形化筛选和排序不会叠加到 SQL 中。MongoDB 请选择“自定义 MongoDB 聚合”，再选择目标数据库和源 Collection，输入 JSON 数组形式的聚合管道；支持 `$lookup`，但拒绝 `$out` 与 `$merge` 等写入阶段。
4. 选择导出目录，确认或修改默认文件名：图形化配置为 `{数据库}-{数据表}-{YYYYMMDD}.xlsx`，自定义 SQL 为 `{数据库类型}-query-{YYYYMMDD}.xlsx`（MySQL/MariaDB 统一为 `mysql-query-{YYYYMMDD}.xlsx`），确认最大行数后开始导出。

切换数据库类型不会立即断开当前已连接的数据库。首次切到某个类型时，连接信息、查询配置、导出配置、状态和进度会恢复为该类型的默认值；在尚未重新测试连接前切回实际已连接的类型，会恢复完整配置并继续使用原连接。只有新类型点击“测试并连接”且成功后，才会释放旧连接、清除旧类型的内存配置并切换到新连接；测试失败也会保留旧连接。SQLite 文件路径不会传递给其他数据库类型。

下拉框仅支持点击选择，滚轮会继续滚动页面而不会意外切换配置。窗口激活时按回车可执行“测试并连接”；Windows 启动时会将前台窗口输入法切换为英文模式。

自定义 SQL 会拒绝多语句、写入、DDL、`EXEC` 与 `SELECT INTO`；连接账户仍应授予最小的只读权限。应用不会记录密码、完整连接串、SQL 正文、筛选值或查询结果。
...
- [v1.0.0](verchanglog/v1.0.0.md)：首个正式版本，提供多数据库只读导出、自定义查询与 Windows EXE 发布能力。
